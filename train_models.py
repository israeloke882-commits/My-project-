...
def load_data():
    ...
    datasets = ['dataset/Nazario_5.csv', 'dataset/email_text.csv', 'dataset/phishing_emails.csv']
    all_dfs = []
    
    for ds in datasets:
        if os.path.exists(ds):
            all_dfs.append(pd.read_csv(ds))
            print(f"  - Loaded {ds}")
            
    if not all_dfs:
        raise FileNotFoundError("No datasets found in the 'dataset/' folder. Please check your file names.")
